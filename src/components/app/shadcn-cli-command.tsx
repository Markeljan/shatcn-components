...
export default function ShadcnCliCommand({ component }: { component: string }) {
  ...
  const pnpmCommand = `pnpm dlx shadcn@latest add https://shatcn.dev/r/${component}.json`
  const npmCommand = `npx shadcn@latest add https://shatcn.dev/r/${component}.json`
  const yarnCommand = `npx shadcn@latest add https://shatcn.dev/r/${component}.json`
  const bunCommand = `bunx --bun shadcn@latest add https://shatcn.dev/r/${component}.json`
      ...
      <TabsContent value="pnpm">
        <Pre __rawstring__={pnpmCommand} data-language="bash">
          <code>
            ...
            <span className="text-white/[0.53] ">{` dlx shadcn@latest add https://shatcn.dev/r/${component}.json`}</span>
          </code>
        </Pre>
      </TabsContent>
